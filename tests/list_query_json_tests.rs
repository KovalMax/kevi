...
fn list_filters_with_query_and_emits_json() {
    ...
    // Seed vault
    let entries = vec![
        VaultEntry { label: "alpha".into(), username: Some(SecretString::new("alice".into())), password: SecretString::new("a".into()), notes: None },
        VaultEntry { label: "beta".into(), username: Some(SecretString::new("bob".into())), password: SecretString::new("b".into()), notes: None },
        VaultEntry { label: "gamma".into(), username: None, password: SecretString::new("c".into()), notes: None },
    ];
    ...
    cmd.env("KEVI_PASSWORD", pw)
        ...
        .arg("--query").arg("BeTa") // case-insensitive
        .arg("--show-users")
        .arg("--json");
