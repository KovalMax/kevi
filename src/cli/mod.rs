...
pub async fn run() -> anyhow::Result<()> {
    ...
    match cli.command {
        ...
        Commands::Get { key, path, field, no_copy, echo, ttl, once } => {
            let config = Config::create(path.map(PathBuf::from));
            let vault = Vault::create(&config);
            ...
            vault.handle_get(&key, field_core, no_copy, ttl, echo, once).await?
        }
        Commands::Add { path, generate, length, no_lower, no_upper, no_digits, no_symbols, allow_ambiguous, passphrase, words, sep, label, user, notes } => {
            let config = Config::create(path.map(PathBuf::from));
            let vault = Vault::create(&config);
        ...
        Commands::List { path, show_users, query, json } => {
            let config = Config::create(path.map(PathBuf::from));
            let vault = Vault::create(&config);
